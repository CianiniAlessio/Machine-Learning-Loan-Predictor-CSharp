...
Defining the Hessian matrix H after some calculation which are easy to find on the internet we obtain that $$H(\theta,\hat{y}) = Y^TQ(\theta,\hat{y}Y$$ where $$Q(\theta,\hat{y}) := diag{\sigma(\theta^T\hat{y(1))(1-\sigma(\theta^T\hat{y}(1))})}),...,\sigma(\theta^T\hat{y}(N))(1-\sigma(\theta^T\hat{y}(N))$$
